...
export default function ChatRoom({ me, other }: Props) {
  ...
  // Cleanup old messages at most once per session, not on every send
  ...
  const handleSend = useCallback(async () => {
    ...
    const { data: newMsg } = await supabase
      .from('messages')
      .insert({ sender_id: me.id, receiver_id: other.id, content })
      .select()
      .single();
    ...
    if (newMsg) {
      ...
      // Delete expired messages once per session, not on every send
      if (!cleanupDoneRef.current) {
        cleanupDoneRef.current = true;
        supabase.from('messages').delete().lt('expires_at', new Date().toISOString()).then();
      }
    }
  }, [inputText, me.id, other.id, supabase]);

  return (
    <div
      className={`flex flex-col h-full bg-beige-50 transition-all duration-300 ${!isFocused ? 'blur-sm brightness-75' : ''}`}
      aria-label="Chat"
    >
      ...
      {!isFocused && (
        <div
          role="alert"
          aria-live="assertive"
          className="absolute inset-0 flex items-center justify-center bg-beige-50/20 backdrop-blur-sm z-50"
